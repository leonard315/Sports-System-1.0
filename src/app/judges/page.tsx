...
import { Plus, Trash2, Edit2, ArrowLeft, Gavel, Search, X, CheckCircle2, Clock } from "lucide-react";
...
export default function JudgesPage() {
  ...
  // For each judge, find which events they are assigned to (by name match in event.judges array)
  const getAssignedEvents = (judgeName: string) =>
    (events || []).filter((e) =>
      e.judges.some((jn) => jn.toLowerCase() === judgeName.toLowerCase())
    );

  // Check if a judge has submitted scores in an event (event status = completed or ongoing)
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-slate-400 font-medium mt-1">Manage the judges registry and track scoring progress per event.</p>
          </div>
          <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
            ...
            <DialogContent className="rounded-[2rem] p-8 border-none premium-shadow bg-slate-900">
              ...
              <div className="py-6 space-y-4">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black uppercase text-slate-500 tracking-widest">Email (optional)</label>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Total Events", value: events?.length || 0, color: "text-indigo-400" },
                  ...
                  return (
                    <TableRow key={judge.id} className="group transition-colors border-slate-800/50 hover:bg-slate-800/30">
                      ...
                      <TableCell className="text-center">
                        {assigned.length === 0 ? (
                          <span className="text-slate-600 text-xs font-bold">None assigned</span>
                        ) : (
